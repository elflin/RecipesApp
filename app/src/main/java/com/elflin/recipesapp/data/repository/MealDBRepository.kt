...
class MealDBRepository(private val service: MealDBService) {

    suspend fun DataForRecipeListView(jumlahData: Int = 15): List<Meal> {
    ...
    suspend fun DataForRecipeDetailView(id: Int): Meal {
        val dataMeal = service.SearchMealbyId(id)
        lateinit var returnMeal: Meal

        if (dataMeal.code() == HttpURLConnection.HTTP_OK) {
            val body = dataMeal.body()!!.meals[0]
            returnMeal = Meal(
                meal = body.strMeal,
                category = body.strCategory,
                area = body.strArea,
